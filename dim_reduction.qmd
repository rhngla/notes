...
bibliography: dim_reduction.bib
...
These notes cover some fundamental concepts relevant to distortion in the context of dimensionality reduction.
...
### Euclidian space {#sec-euc_manifold}
...
Textbooks on differential geometry, e.g. @andrews2020dg would provide a more formal entry point and description for Manifolds. For this article we'll restrict discussion to _Reimannian_ manifolds, which may be thought of as a generalization of Euclidean space. In particular, Reimannian manifolds are _locally_ Euclidean, and I think of it as a smoothly warped version of Euclidean space.

### Notation
More formally, a Riemannian manifold is a tuple $(M,g)$ where $M$ is a set of points and $g$ refers to inner products on vector spaces that we define at each point $p \in M$. The vectors that are tangent to the point $p$ make up the tangent space denoted by $T_{p} M$. We thus denote the inner product at $p \in M$ as the map: $g_{p}: T_{p} M \times T_{p} M \rightarrow \mathbb{R}$.

Let the local co-ordinates 
...
# Make bibtex entry out of the following:

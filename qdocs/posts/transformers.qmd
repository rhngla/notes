...
@phuong2022formal are perhaps the best resource for consistent notation. Here I show a stripped down version of self-attention mainly to highlight how attention captures dependence of the output on particular entries of the input sequence.

We denote the $i$-th input as $x_i \in \mathbb{R}^{d_{\textrm{in}}}$. Similarly the $j$-th output $y \in \mathbb{R}^{d_{\textrm{out}}}$. Let length of the sequence $T$.
...
$K, Q \in \mathbb{R}^{d_\textrm{key-query} \times d_{\textrm{in}}}$ and $V \in \mathbb{R}^{d_\textrm{out} \times d_{\textrm{in}}}$, are trainable weights. $A$ is known as the attention matrix. Setting certain entries of the $A_{ij}$ to zero is referred to as masking, and it has the effect of preventing interaction between the $i$-th and $j$-th inputs in constructing the $i$-th output.

For language, setting the upper triangular entries (excluding the diagonal) to zero is a common choice. This mask pattern ensures that $y_i$ is only based on previous and current inputs, i.e. $(x_1, ..., x_{i})$.
...
::: {.column-margin}
I highly recommend the implementation walkthrough by Karpathy [-@karpathy2023llm]. He's a skilled teacher and watching him live-code is instructive.
:::

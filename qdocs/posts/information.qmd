...
## Monte Carlo estimate of Expectations
 ...
 - Monte Carlo integration allows us to approximate integrals over regions.
 - In particular, for regions that are valid probability distributions:

\begin{aligned}
\int f(x) p(x) dx \approx \frac{1}{N} \sum_{i=1}^N f(x_i) \\
\end{aligned}

 - Expectations over probability distributions can therefore be expressed as sums over samples!
 - The larger $N$ is, the better the approximation is.
 - We often assume that $p$ is simple-enough distribution (e.g. low dimensionality) so that a relatively small number of samples is sufficient to approximate such integrals well.

 <p></p>
 
 - Consider cross entropy, where the integral is over the distribution $p$.
 - We don't have access to $p$ directly, only `observations`, that we assume to be independently drawn from the same distribution $p$.
 - Using $i$ to indicate distinct observations, $x_i \sim p$, we can approximate cross-entropy as a sum over `observations`:
...
Re. entropy:
 ...
 - The lower the entropy, the less surprising are samples on average.
 ...
 - A smore tructured system means the related distribution has lower entropy
...
A numerical example with Gaussians:
 ...
 - The goal is simply  to compare entropy-based quantities, which serve as motivation for such algorithms.
...
[Shreyas Potnis](https://scholar.google.com/citations?user=WxTzovMAAAAJ) and [Priyanka Kulkarni](https://scholar.google.com/citations?hl=en&user=ViilpdV5QxsC) for helpful comments and suggestions.